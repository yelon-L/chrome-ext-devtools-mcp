...
const TABLE_MAPPINGS = [
  // ... 现有配置
  {
    ...
    columns: [
      {dbName: 'product_id', tsName: 'productId', type: 'number', nullable: false},
...
// 类型自动生成，100% 准确
const product = await db.selectFrom('mcp_products')
  .selectAll()
  .executeTakeFirst();
...
const TABLE_MAPPINGS = [
  {
    ...
    columns: [
      // ... 现有列
      {dbName: 'avatar_url', tsName: 'avatarUrl', type: 'string', nullable: true},
    ],
  },
];
```
