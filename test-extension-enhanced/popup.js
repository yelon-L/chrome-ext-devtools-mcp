...
// 生命周期测试标记（供 MCP 工具验证使用）
window.popupLifecycleTestMarker = {
  opened: Date.now(),
  version: '1.0.0',
  testId: `popup_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
};

console.log('[Popup] 🏷️ Life周期标记已设置:', window.popupLifecycleTestMarker);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // 生命周期测试：ping 命令（检测 popup 是否存活）
  if (message.type === 'ping_popup') {
    console.log('[Popup] 响应 ping');
    sendResponse({
      alive: true,
      timestamp: Date.now(),
      marker: window.popupLifecycleTestMarker,
    });
    return true;
  }
