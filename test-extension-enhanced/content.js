...
class ContentScriptTester {
  ...
  async init() {
    ...
    performance.measure(
      'content-script-init',
      'content-script-init-start',
      'content-script-init-end',
    );

    const initMeasure = performance.getEntriesByName('content-script-init')[0];
    console.log(
      '[Enhanced Content] ⏱️ 初始化耗时:',
      initMeasure.duration,
      'ms',
    );
  ...
  setupDOMObserver() {
    ...
    const observer = new MutationObserver(mutations => {
      const addedNodes = mutations.reduce(
        (sum, mut) => sum + mut.addedNodes.length,
        0,
      );
      const removedNodes = mutations.reduce(
        (sum, mut) => sum + mut.removedNodes.length,
        0,
      );
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'tab-loaded') {
    console.log(
      '[Enhanced Content] 📨 Week 4: 收到background消息:',
      message.type,
    );
...
class ContentPerformanceTester {
  ...
  executePerformanceTest(operations, level) {
    this.isActive = true;
    console.log(
      `[Enhanced Content] 🎯 执行性能测试 - 操作数: ${operations}, 级别: ${level}`,
    );
  ...
  triggerPaintOperations(count) {
    ...
    for (let i = 0; i < count; i++) {
      if (this.testElements[i]) {
        // 改变视觉属性触发Paint
        this.testElements[i].style.backgroundColor =
  ...
  executeJavaScriptWork(level) {
    ...
    console.log(
      `[Enhanced Content] ⚡ JavaScript计算完成: ${elapsed.toFixed(2)}ms, 结果: ${result.toFixed(2)}`,
    );
  }
...
document.documentElement.setAttribute(
  'data-mcp-performance-test-ready',
  'true',
);

console.log(
  '[Enhanced Content] ✅ v4.1加载完成 - Week 1-4全功能 + Phase 1性能测试就绪',
);
