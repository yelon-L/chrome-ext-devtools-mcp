...
class MCPTester {
  ...
  async runAllTests() {
    ...
    await this.testTool('Interaction', 'resize_page', {
      width: 1280,
      height: 720,
    });
  ...
  showResults() {
    ...
    console.log(
      `📊 成功率: ${((this.results.passed.length / this.results.total) * 100).toFixed(1)}%`,
    );
