...
console.log(
  '║              MCP IDE Simulator - Protocol Flow                 ║',
);
console.log(
  '╚════════════════════════════════════════════════════════════════╝\n',
);
              ...
              if (dataMatch) {
                ...
                if (!sessionId && dataStr.includes('/message?sessionId=')) {
                  ...
                  if (sidMatch) {
                    ...
                    console.log(
                      `   This is how IDE gets the session for subsequent calls\n`,
                    );
...
async function ideWorkflow(sseReq, done) {
  try {
    ...
    console.log(
      'IDE sends initialize to negotiate protocol version and capabilities',
    );
    ...
    const extensionTools = availableTools.filter(t =>
      t.name.includes('extension'),
    );
    const browserTools = availableTools.filter(
      t => !t.name.includes('extension'),
    );
    ...
    console.log(
      'IDE: "User wants to debug extensions, let me see what\'s installed"',
    );
    console.log(
      'IDE: "First, I\'ll try the standard call to see enabled extensions"',
    );
    ...
    if (listResult.result?.content) {
      ...
      if (extMatches) {
        ...
        const hasEnhancedMCP = allExtensions.some(
          id => text.includes(id) && text.includes('Enhanced MCP'),
        );
        const hasVideoSRT = allExtensions.some(
          id => text.includes(id) && text.includes('Video SRT'),
        );

        console.log('\n📊 Target Extensions Check:');
        console.log(
          `   ${hasEnhancedMCP ? '✅' : '❌'} Enhanced MCP Debug Test Extension`,
        );
        ...
        console.log(
          '   This means the visual detection fallback may have been used',
        );
      }
    }
    ...
    if (foundExtensions.length === 0) {
      ...
      console.log(
        '   Option 1: Navigate to chrome://extensions/ and screenshot',
      );
      ...
      console.log(
        `🤔 IDE: Let me check if Service Worker is active for ${testExtId.substring(0, 12)}...`,
      );
      ...
      if (contextsResult.result?.content) {
        ...
        if (!hasSW) {
          ...
          console.log(
            '   (This is prerequisite awareness from tool descriptions)',
          );
    ...
    console.log(
      '   Problem: list_extensions cannot detect disabled/inactive extensions',
    );
    console.log(
      '   IDE Solution: Falls back to visual inspection automatically',
    );
