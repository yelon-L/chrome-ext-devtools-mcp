...
export function displayStdioModeInfo(): void {
  ...
  console.error(LINE);
  console.error('This MCP server provides full access to browser debugging capabilities.');
  ...
  console.error('   Multi-tenant SaaS:  node build/src/multi-tenant/server-multi-tenant.js');
  console.error(LINE);
  console.error('');
}
...
export function displayMultiTenantModeInfo(port: number): void {
  ...
  console.error('        -d \'{"userId":"alice","browserURL":"http://localhost:9222"}\'');
  console.error('');
