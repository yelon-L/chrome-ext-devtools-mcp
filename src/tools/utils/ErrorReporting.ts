...
import {formatErrorForUser} from '../../config/ErrorVerbosity.js';
...
export function reportExtensionNotFound(
  ...
  response.appendResponseLine(
    `${ErrorSeverity.ERROR} **${ErrorType.NOT_FOUND}**: Extension not found\n`,
  );
  response.appendResponseLine(`**Requested ID**: \`${extensionId}\`\n`);

  if (availableExtensions.length === 0) {
    response.appendResponseLine(
      '⚠️ **No extensions installed** in this Chrome instance\n',
    );
    response.appendResponseLine('**Suggestions**:');
    response.appendResponseLine('1. Install the extension in Chrome');
    response.appendResponseLine(
      '2. Make sure Chrome is connected via remote debugging',
    );
    response.appendResponseLine(
      '3. Check if the extension is loaded in chrome://extensions\n',
    );
  } else {
    response.appendResponseLine(
      `**Available extensions** (${availableExtensions.length} total):\n`,
    );
    ...
    if (similar) {
      response.appendResponseLine(
        `💡 **Did you mean**: \`${similar.id}\` (${similar.name})?\n`,
      );
    }
  }
...
export function reportNoBackgroundContext(
  ...
  response.appendResponseLine(
    `${ErrorSeverity.ERROR} **${ErrorType.PRECONDITION_FAILED}**: Background context not available\n`,
  );
  ...
  if (extension) {
    ...
    if (extension.serviceWorkerStatus) {
      response.appendResponseLine(
        `**Service Worker Status**: ${extension.serviceWorkerStatus}\n`,
      );
    }
  }
  ...
  if (extension?.manifestVersion === 3) {
    ...
    response.appendResponseLine(
      '   - MV3 Service Workers become inactive after inactivity',
    );
  ...
  response.appendResponseLine(
    '   - Shows all active contexts (background, popup, content scripts)',
  );
  response.appendResponseLine(
    '3. **Diagnose errors**: `diagnose_extension_errors`',
  );
  response.appendResponseLine(
    '   - Checks for crash logs and startup failures',
  );
...
export function reportTimeout(
  ...
  response.appendResponseLine(
    `⏱️ **${ErrorType.TIMEOUT}**: ${operationName} exceeded time limit\n`,
  );
  ...
  response.appendResponseLine(
    '1. Operation is taking longer than expected (may still complete)',
  );
  ...
  response.appendResponseLine('**Suggestions**:');
  response.appendResponseLine(
    '1. Wait a few more seconds - operation may still complete',
  );
  response.appendResponseLine(
    '2. Check extension status with `list_extensions`',
  );
  response.appendResponseLine(
    '3. Try the operation again without the timeout option',
  );
...
export function reportResourceUnavailable(
  ...
  response.appendResponseLine(
    `${ErrorSeverity.WARNING} **${ErrorType.UNAVAILABLE}**: ${resourceType} not available\n`,
  );
  ...
  if (resourceType === 'Manifest') {
    ...
    response.appendResponseLine(
      'Extension manifest data is loaded asynchronously from Chrome. On first access, the data may not be ready yet.\n',
    );
    ...
    response.appendResponseLine(
      `get_extension_details(extensionId="${resourceId}")`,
    );
    response.appendResponseLine('');
    response.appendResponseLine(
    ...
    response.appendResponseLine(
      `inspect_extension_manifest(extensionId="${resourceId}")`,
    );
    response.appendResponseLine('```');
  } else {
    // 通用建议（用于其他资源类型）
...
export function reportNoDialog(response: Response): void {
  response.appendResponseLine(
    `${ErrorSeverity.INFO} **No open dialog found**\n`,
  );
  response.appendResponseLine(
    'There is currently no active browser dialog to handle.\n',
  );
