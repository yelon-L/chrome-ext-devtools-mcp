...
- \`enhance_extension_error_capture\` - Proactive error capture`,
  ...
  schema: {
    extensionId: z
      .string()
      .regex(/^[a-z]{32}$/)
      .describe('Extension ID to get runtime errors from. Get this from list_extensions.'),
    ...
    sortBy: z
      ...
      .optional()
      .describe('Sort errors by occurrence count or timestamp. Default is occurrences.'),
    limit: z
      .number()
      .positive()
      .optional()
  ...
  handler: async (request, response, context) => {
    ...
    try {
      ...
      response.appendResponseLine(`## 🔍 Extracting Errors from chrome://extensions\n`);
      ...
      let manifestErrors: any[] = [];
      let installWarnings: any[] = [];
      let errorCount = 0;
      ...
      try {
        ...
        const errorData = await page.evaluate(async (extId: string) => {
          ...
          if (typeof chromeAPI === 'undefined' || !chromeAPI.developerPrivate) {
            return {error: 'chrome.developerPrivate API not available', errors: []};
          }
          ...
          return new Promise((resolve) => {
            try {
              ...
              chromeAPI.developerPrivate.getExtensionsInfo({
                ...
                const targetExt = extensions.find((ext: any) => ext.id === extId);
                
                if (!targetExt) {
                  resolve({error: 'Extension not found in developerPrivate data', errors: []});
                  return;
                }
                
                // Extract error information
                const runtimeErrors = (targetExt.runtimeErrors || []).map((err: any) => ({
                  message: err.message || '',
                  source: err.source || '',
                  stack: err.stackTrace ? err.stackTrace.map((frame: any) => {
                  ...
                  context: err.contextUrl || '',
                  type: err.type || 'runtime',
                  occurrences: err.occurrences || 1,
                  severity: err.severity || 'ERROR',
                  renderProcessId: err.renderProcessId,
                  renderViewId: err.renderViewId,
                }));
                
                const manifestErrors = (targetExt.manifestErrors || []).map((err: any) => ({
                  message: err.message || '',
                  source: err.manifestKey || '',
                  type: 'manifest',
                }));
                
                const installWarnings = (targetExt.installWarnings || []).map((warn: any) => 
                  typeof warn === 'string' ? warn : warn.message || ''
                );
        ...
        if (typedErrorData.error) {
          response.appendResponseLine(`⚠️ **Unable to extract errors**: ${typedErrorData.error}\n`);
        } else {
          ...
          if (typedErrorData.usedDeveloperPrivate) {
            response.appendResponseLine(`✅ **Successfully used chrome.developerPrivate API** to extract complete error details\n`);
          }
        }
      ...
      if (!includeWarnings) {
        runtimeErrors = runtimeErrors.filter((err: any) => err.severity !== 'WARNING');
      }
      ...
      if (errorCount > 0) {
        response.appendResponseLine(`- 🔴 **Error Count**: ${errorCount} (from chrome://extensions)`);
      }
      
      response.appendResponseLine(`- 📝 **Extracted Details**: ${runtimeErrors.length} error(s)`);
      
      if (includeManifestErrors && manifestErrors.length > 0) {
        response.appendResponseLine(`- 📦 **Manifest Errors**: ${manifestErrors.length}`);
      }
      
      if (installWarnings.length > 0) {
        response.appendResponseLine(`- ⚠️ **Install Warnings**: ${installWarnings.length}`);
      }

      response.appendResponseLine('');

      // 5. Handle no errors case
      if (errorCount === 0 && runtimeErrors.length === 0 && manifestErrors.length === 0) {
        response.appendResponseLine('✅ **No errors found!**\n');
        response.appendResponseLine('The extension appears to be running correctly according to chrome://extensions.\n');
      ...
      if (errorCount > 0 && runtimeErrors.length === 0) {
        ...
        response.appendResponseLine(`Chrome shows **${errorCount} errors** for this extension, but error details are not accessible without user interaction.\n`);
      ...
      if (sortBy === 'occurrences') {
        runtimeErrors.sort((a: any, b: any) => (b.occurrences || 0) - (a.occurrences || 0));
      }
      ...
      if (displayErrors.length > 0) {
        response.appendResponseLine(`## Runtime Errors (${displayErrors.length})\n`);

        displayErrors.forEach((error: any, index: number) => {
      ...
      if (includeManifestErrors && manifestErrors.length > 0) {
        ...
        manifestErrors.forEach((error: any, index: number) => {
          response.appendResponseLine(`### 📦 Manifest Error #${index + 1}\n`);
          response.appendResponseLine(`**Message**: ${error.message}`);
          
          if (error.manifestKey) {
            response.appendResponseLine(`**Manifest Key**: \`${error.manifestKey}\``);
          }
      ...
      if (installWarnings.length > 0) {
        ...
        installWarnings.forEach((warning: any, index: number) => {
          response.appendResponseLine(`### ⚠️ Warning #${index + 1}\n`);
          response.appendResponseLine(`**Message**: ${warning.message || warning}\n`);
        });
      }

      // 13. Note about implementation status
      response.appendResponseLine(`## 💡 Note\n`);
      response.appendResponseLine('This tool is still under development. Once fully implemented, it will provide:');
      response.appendResponseLine('- Error occurrence counts to identify high-frequency issues');
      response.appendResponseLine('- Complete stack traces from Chrome\'s internal records');
      response.appendResponseLine('- Detailed recommendations based on error patterns\n');
