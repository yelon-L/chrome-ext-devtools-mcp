...
export async function handleListUsersV2(
  ...
  const usersWithBrowserCount = await Promise.all(
    users.map(async (user: UserRecordV2) => {
...
export async function handleBindBrowserV2(
  ...
  try {
    ...
    // 绑定浏览器（浏览器已验证可达）
    const browser = await this.getUnifiedStorage().bindBrowser(
      userId,
      browserURL,
      finalTokenName,
      description,
    ...
    if (browserDetection.browserInfo) {
      await this.getUnifiedStorage().updateBrowser(browser.browserId, {
        description:
          description || `Chrome ${browserDetection.browserInfo.browser}`,
      });
    }
    ...
    JSON.stringify(
      {
        browsers: browsers.map((b: BrowserRecordV2) => ({
          ...
          lastConnectedAt: b.lastConnectedAt
            ? new Date(b.lastConnectedAt).toISOString()
            : null,
        })),
    ...
    JSON.stringify(
      {
        ...
        metadata: {
          ...
          createdAt: new Date(browser.createdAt).toISOString(),
          lastConnectedAt: browser.lastConnectedAt
            ? new Date(browser.lastConnectedAt).toISOString()
            : null,
        },
      },
      null,
...
export async function handleUpdateBrowserV2(
  ...
  try {
    ...
    const updatedBrowser = await this.getUnifiedStorage().getBrowserAsync(
      browser.browserId,
    );
    if (!updatedBrowser) {
      res.writeHead(404, {'Content-Type': 'application/json'});
